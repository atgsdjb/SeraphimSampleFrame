...
public class UuseeDeviceActivity extends Activity {
	static  private final String TAG =UuseeGlobal.TAG;
...
//	private TextView titleText;
	...
	private class NotivyReceiver extends BroadcastReceiver{
		...
		public void onReceive(Context context, Intent intent) {
			...
			String msg = intent.getStringExtra(UuseeGlobal.BORADCASET_NOTITY_DATA_KEY);
		...
		public void onClick(View v) {
			...
			case R.id.show_all_action:
				...
				for(int i = 0;i<al.size();i++){
					...
					String log = UuseeUpnpStaticTools.getActionInfo(a);
					Log.e(TAG,log);
				}
				break;
			case R.id.clos_sulf:
...
//				mCP.sendAction(currentService, "SetAVTransportURI", arg);
				mDialog.dismiss();
				Intent intent = new Intent(UuseeDeviceActivity.this,UuseeUpnpBrowseActivity.class);
				String type = currentService.getServiceType();
				String udn = mDevice.getUDN();
				intent.putExtra(UuseeGlobal.KEY_DEVICE_UDN, udn);
				intent.putExtra(UuseeGlobal.KEY_SERVER_TYPE,type);
...
//		unregisterReceiver(mNotivyReceiver);
	...
	private void initElement(){
		...
		filter.addAction(UuseeGlobal.BROADCASET_REVICE_NOTITY_ACTION);
...
//		titleText.setText(name+"#"+location);
	...
	private void initData() {
		...
		String l_location = intent.getStringExtra(UuseeGlobal.KEY_DEVICE_LOCATION);
		String l_name = intent.getStringExtra(UuseeGlobal.KEY_DEVICE_NAME);
		String l_udn = intent.getStringExtra(UuseeGlobal.KEY_DEVICE_UDN);
		if(l_name != null)
			name = l_name;
		if(l_udn!=null)
			udn = l_udn;
